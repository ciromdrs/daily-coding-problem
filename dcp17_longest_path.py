...
example above, the longest absolute path is "dir/subdir2/subsubdir2/
...
def count(l):
    length = 0
    for s in l:
        length += len(s) + 1 # +1 for the slash
    length -= 1 # remove dangling slash
    return length

fs = "dir\n\tsubdir1\n\t\tfile1.ext\n\t\tsubsubdir1\n\tsubdir2\n\t\tsubsubdir2\n\t\t\tfile2.ext"

def longest_path_to_file(fs):
    lines = fs.split('\n')
    longest = ''
    old_level = 0
    path = []
    for l in lines:
        ...
        if level >= old_level:
            ...
            path = path[:-(old_level-level)]
        old_level = level
        print(l,level)
    print(len(longest),longest)

longest_path_to_file(fs)
